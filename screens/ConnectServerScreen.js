    ...
    <View style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
      ...
      {loading ? (
        ...
        <>
          {state.accountList.length > 0 && (
            <View
              ...
              <FlatList
                ...
                renderItem={({item: [id, accountInfo]}) => (
                  <ListItem
                    id={id}
                    accountInfo={accountInfo}
                    navigation={navigation}
                    setLoading={setLoading}
                    updateActiveAccount={(activeAccount) => dispatch({type: 'login', payload: activeAccount})}
                  />
                )}
                keyExtractor={([id]) => id}
              />
          ...
          <Button title="Add Server" onPress={() => navigation.navigate('ServerSettings')} />
        </>
      )}
    </View>
  );
