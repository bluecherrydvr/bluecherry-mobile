...
export async function setAccountInfo(accountId, accountInfo) {
  ...
  const setAccountId = () => AsyncStorage.setItem('account_' + accountId, JSON.stringify(accountInfo));

  if (ids.includes(accountId)) {
    return setAccountId();
  }

  return Promise.all([AsyncStorage.setItem('account_ids', JSON.stringify(ids.concat(accountId))), setAccountId()]);
}
...
export async function removeAccountInfo(accountId) {
  ...
  return Promise.all([
    ...
    removeSelectedCameraList(accountId),
    async () => {
      const activeAccount = await getActiveAccount();
      if (activeAccount === accountId) {
        return removeActiveAccount();
      }
    },
  ]);
}
...
export async function getAccountInfoList() {
  ...
  return Promise.all(accountIds.map(async (id) => [id, await getAccountInfo(id)]));
}
...
async function removeActiveAccount() {
  return AsyncStorage.removeItem('active_account');
}
...
export async function getTargetAccountByServerId(accountList, serverId) {
  for (const info of accountList) {
    if (serverId === info[1].serverUuid) {
      return info;
    }
  }
}

export async function removeSelectedCameraList(accountId) {
  return AsyncStorage.removeItem('selected_cameras_' + accountId);
}
