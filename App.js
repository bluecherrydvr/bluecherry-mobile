    ...
    updateAccountList().then(async (accountList) => {
      if (!state.activeAccount) {
        ...
        if (initialNotification) {
          ...
          loginTargetAccount(
            await getTargetAccountByServerId(accountList, serverId),
      ...
      Promise.all([getAvailableDevices(state.activeAccount), getSelectedCameraList(state.activeAccount.id)]).then(
        ([deviceList, selectedDeviceList]) => {
          dispatch({
            type: 'update_all_device_list',
            deviceList,
            selectedDeviceList,
          });
        ...
        <NavigationContainer ref={navRef} theme={DarkTheme}>
          {state.activeAccount ? (
            <Drawer.Navigator drawerContent={(props) => <DrawerMenu {...props} />}>
              ...
              <Drawer.Screen
                name="DirectCamera"
                component={DirectCameraScreen}
                options={{title: 'Direct Camera'}}
                listeners={{
                  focus: () => {
                    SplashScreen.hide();
                    Orientation.lockToLandscape();
